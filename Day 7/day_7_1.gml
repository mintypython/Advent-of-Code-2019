...
ds_list_add(sequence_list, 0, 1, 2, 3, 4);
var n = ds_list_size(sequence_list);
var c = [0,0,0,0,0];

var max_output = amplifier_control_software(memory, sequence_list);
...
while(i < n){
	if(c[i]<i){
		...
		max_output = max(max_output, amplifier_control_software(memory, sequence_list));
